...
Emits structured diagnostic advisories when internal I/O bottlenecks occur to prevent false-positive PoW lockouts.

HONESTY NOTICE:
This classifier is a calibrated, rule-based heuristic decision engine tuned against
...
class TrafficClassifier:
    ...
    def classify(self, metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 1. Baseline: Minimal traffic and nominal response
        ...
        # 2. Downstream / DB Stall Signal:
        # High latency (> 180ms), but CPU is completely flat / low (< 40%) and per-request CPU time is low (< 5ms).
        # PoW must be SUPPRESSED here because the bottleneck is I/O / DB wait, not computational exhaustion!
        ...
        # 3. Complexity Attack Signal:
        # High CPU stress (either system CPU >= threshold OR mean CPU time per request > 10ms),
        # heavy concentration (> 65%) on the expensive /verify-crypto endpoint,
        ...
        # 4. Benign Traffic Surge Signal:
        # High aggregate RPS, but load is distributed across light endpoints (concentration <= 45%),
